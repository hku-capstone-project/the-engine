...
Renderer::Renderer(VulkanApplicationContext *appContext, Logger *logger, size_t framesInFlight,
  ...
  _pipeline = std::make_unique<GfxPipeline>(appContext, logger, WorkGroupSize{8, 8, 8},
                                            _descriptorSetBundle.get(), shaderCompiler);
...
void Renderer::_createDepthStencil() {
  ...
  createInfo.extent      = {_appContext->getSwapchainExtent().width,
  ...
  allocateInfo.memoryTypeIndex = _appContext->getMemoryType(memoryRequirements.memoryTypeBits,
...
void Renderer::drawFrame(size_t currentFrame) {
  ...
  vkCmdBindIndexBuffer(_tracingCommandBuffers[currentFrame], _model->indexBuffer->getVkBuffer(), 0,
  ...
  // vkCmdBindDescriptorSets(_tracingCommandBuffers[currentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS,
