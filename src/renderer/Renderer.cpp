...
Renderer::Renderer(VulkanApplicationContext *appContext, Logger *logger,
  ...
  _images.baseColor =
            std::make_unique<Image>(_appContext, _logger,
...
void Renderer::_createDepthStencil() {
    ...
    createInfo.format = _appContext->getDepthFormat();
    createInfo.extent = {_appContext->getSwapchainExtent().width, _appContext->getSwapchainExtent().height, 1};
    createInfo.mipLevels = 1;
    createInfo.arrayLayers = 1;
    createInfo.samples = _appContext->getMsaaSample();
    createInfo.tiling = VK_IMAGE_TILING_OPTIMAL;
    createInfo.usage = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;

    vkCreateImage(_appContext->getDevice(), &createInfo, nullptr, &depthStencil.image);

    VkMemoryRequirements memoryRequirements{};
    vkGetImageMemoryRequirements(_appContext->getDevice(), depthStencil.image, &memoryRequirements);

    VkMemoryAllocateInfo allocateInfo{};
    allocateInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocateInfo.allocationSize = memoryRequirements.size;
    allocateInfo.memoryTypeIndex = _appContext->getMemoryType(memoryRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    vkAllocateMemory(_appContext->getDevice(), &allocateInfo, nullptr, &depthStencil.memory);
    vkBindImageMemory(_appContext->getDevice(), depthStencil.image, depthStencil.memory, 0);
...
void Renderer::drawFrame(size_t currentFrame) {
    ...
    vkCmdBeginRenderPass(_tracingCommandBuffers[currentFrame], &rdrPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);

    VkDeviceSize offsets[] = {0};
    vkCmdBindVertexBuffers(_tracingCommandBuffers[currentFrame], 0, 1, &_model->vertexBuffer->getVkBuffer(), offsets);
    vkCmdBindIndexBuffer(_tracingCommandBuffers[currentFrame], _model->indexBuffer->getVkBuffer(), 0, VK_INDEX_TYPE_UINT32);
    ...
    vkCmdBindPipeline(_tracingCommandBuffers[currentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS, _pipeline->getPipeline());
    // vkCmdBindDescriptorSets(_tracingCommandBuffers[currentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS, _pipeline->getPipelineLayout(), 0, 1, &descriptorSet, 0, nullptr);
...
void Renderer::_recordTracingCommandBuffers() {
    ...
    vkAllocateCommandBuffers(_appContext->getDevice(), &allocateInfo, _tracingCommandBuffers.data());
}
